...
void Xcp_negative_response(uint8_t error_code) {
    ...
    xcp_send_frame.Cs = Checksum(xcp_send_frame); // Calculate checksum if needed
}

uint8_t Checksum(XCP_sxl_Frame xcp_sxl_Frame){
    uint8_t checksum = 0;
    checksum += xcp_sxl_Frame.Len;
    checksum += xcp_sxl_Frame.Ctr;
    for (int i = 0; i < xcp_sxl_Frame.Len - 2; i++) { // Exclude Len and Ctr from checksum calculation
        checksum += xcp_sxl_Frame.Data[i];
    }
    return checksum;
}
...
void Xcp_connect() {
        ...
        xcp_send_frame.Cs = Checksum(xcp_send_frame); // Calculate checksum for the connect packet
...
void Xcp_disconnect() {
    ...
    xcp_send_frame.Cs = 0; // Calculate checksum for the disconnect packet if needed
}
...
void Xcp_get_status() {
    ...
    if(xcp_com_status == Connected) {
        ...
        xcp_send_frame.Data[1] = CURRENT_RESOURCE_PROTECTION_STATUS;
        ...
        xcp_send_frame.Cs = Checksum(xcp_send_frame); // Calculate checksum for the get status packet
    } else {
        Xcp_negative_response(ERR_CMD_UNKNOWN); // Send negative response if not connected
    }
}
